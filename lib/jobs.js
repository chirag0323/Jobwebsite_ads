...
// New Vercel Blob projects use OIDC credentials and expose BLOB_STORE_ID rather
// than a long-lived BLOB_READ_WRITE_TOKEN. The Blob SDK authenticates both.
...
async function getBlobJobs() {
  ...
  const jobs = await Promise.all(blobs.map(async (blob) => {
    ...
    return { ...(await new Response(result.stream).json()), blobUrl: blob.url };
  }));
